...
import { chargeDeleteAgency } from "../../../../store/slices/agencies/chargeAgenciesSlice";
...
const ChargeAgenciesTabelBody = () => {
    ...
    useEffect(() => {
        if (hideDeleteModel && !chargeAgencies.ChargeDeleteAgencyLoading) {
            setHideDeleteModel(true);
            setShowDeleteModel({ type: false, agency_id: 0 });
        }
    }, [hideDeleteModel, chargeAgencies.ChargeDeleteAgencyLoading]);
    ...
    // console.log(
    //     chargeAgencies.ChargeIndexCopyData?.data[0].owner.profile_image
    // );

    return (
        <>
            {chargeAgencies.ChargeIndexCopyData?.data &&
            chargeAgencies.ChargeIndexCopyData?.data.length > 0 ? (
                <tbody className='font-light text-sm'>
                    {chargeAgencies.ChargeIndexCopyData?.data.map(
                        (agency, i) => (
                            <tr
                                key={i}
                            ...
                            >
                                <td className='capitalize text-center py-3 pl-1 sm:pl-3 text-[10px] sm:text-base border-r-[1px] border-white'>
                                    {i +
                                        1 +
                                        chargeAgencies.ChargeIndexCopyData
                                            ?.per_page! *
                                            (+chargeAgencies.ChargeIndexCopyData
                                                ?.current_page! -
                                                1)}
                                ...
                                <td className='capitalize text-center py-3 pl-1 sm:pl-3 text-[10px] sm:text-base border-r-[1px] border-white hidden lg:table-cell'>
                                    {agency.charge_agent_no}
                                </td>
                                <td className='capitalize text-center py-3 pl-1 sm:pl-3 text-[10px] sm:text-base border-r-[1px] border-l-[1px] border-white'>
                                    {ConvertNumberWishK_M(agency.balance, 1)}
                                </td>
            ...
            <tfoot>
                <tr>
                    <td>
                        <ConfirmDelete
                            deleteBtnContent={
                                chargeAgencies.ChargeDeleteAgencyLoading
                                    ? "الرجاء الانتظار..."
                                    : "أحذف"
                            }
...
export default ChargeAgenciesTabelBody;
